...
public class XboxGear implements Updatable {
	...
	public XboxGear(SingleSolenoid rotateSolenoid,
...
//			setPos(true, false, false);
			if(JoystickIO.xboxController.getRawAxis(2) >= .8) {
				currentState = 1;
				initialTime = Time.getTime();
			} else if(JoystickIO.xboxController.getRawAxis(3) >= .8) {
				currentState = 10;
			}
			break;
		case 1: // Waiting
		...
		case 9:
			if(gearAlignBanner.get() || (Time.getTime() - initialTime >= ROTATE_TIMEOUT)) {
				...
				currentState = 0;
			} else if(JoystickIO.xboxController.getRawAxis(2) >= .8) {
				gearRotatorMotor.set(0.0);
				currentState = 1;
			}
			break;
